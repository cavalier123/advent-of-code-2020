...
//                secondHyperGrid.add(Triple(column, row, 0))
//            }
//        }
//    }
//
...
//    println("---------")
//
//    val adjs = getPointsNear(Triple(2,2,2))
//    for (adj in adjs) {
//        println(adj)
//    }
//
...
//    return (-1 .. 1).flatMap{ d -> (-1 .. 1).map { a -> d to a} }.flatMap{pair -> (-1 .. 1)
//            .map { z -> Triple(pair.first, pair.second, z)} }.filter {it != Triple(0,0,0)}
//            .map {Triple(it.first + point.first, it.second + point.second, it.third + point.third)}
//
//}
//
